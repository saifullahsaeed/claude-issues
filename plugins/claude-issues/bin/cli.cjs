...
function resolvePaths(cwd = process.cwd()) {
  const root = import_node_path.default.join(cwd, ROOT_DIR_NAME);
  const html2 = import_node_path.default.join(root, "_html");
  return {
    cwd,
    root,
    open: import_node_path.default.join(root, "open"),
    fixed: import_node_path.default.join(root, "fixed"),
    archive: import_node_path.default.join(root, "archive"),
    html: html2,
    htmlIndex: import_node_path.default.join(html2, "index.html"),
    index: import_node_path.default.join(root, "INDEX.md")
  };
}
function dirForStatus(paths, status) {
  switch (status) {
    case "open":
      return paths.open;
    case "fixed":
...
function listFixed(paths) {
  return readDir(paths.fixed);
}
function listArchive(paths) {
  return readDir(paths.archive);
}
function listAll(paths) {
...
function regenerateIndex(paths) {
  const open = listOpen(paths).sort(
    (a, b) => SEVERITY_RANK[a.frontmatter.severity] - SEVERITY_RANK[b.frontmatter.severity] || a.frontmatter.id.localeCompare(b.frontmatter.id)
  );
  const fixed = listFixed(paths).sort(
    (a, b) => (b.frontmatter.fixed_at ?? "").localeCompare(a.frontmatter.fixed_at ?? "")
  ).slice(0, 20);
  const archive = listArchive(paths).sort(
    (a, b) => (b.frontmatter.closed_at ?? b.frontmatter.fixed_at ?? "").localeCompare(
...
function renderTableFixed(issues) {
  if (issues.length === 0) return "_None._\n";
...
function renderTableArchive(issues) {
  if (issues.length === 0) return "_None._\n";
...
function joinFiles(i) {
  return i.frontmatter.files?.join(", ") ?? "";
}
function linkSummary(i) {
  const parts = [];
...
${currentText}` : currentText;
      ...
      list(src) {
        ...
        if (cap) {
          ...
          list3.items[list3.items.length - 1].raw = list3.items[list3.items.length - 1].raw.trimEnd();
          list3.items[list3.items.length - 1].text = list3.items[list3.items.length - 1].text.trimEnd();
      ...
      table(src) {
        ...
        for (const row of rows) {
          item.rows.push(splitCells(row, item.header.length).map((cell2, i) => {
            return {
              text: cell2,
...
`;
      ...
      table(token) {
        ...
        for (let j = 0; j < token.header.length; j++) {
          cell2 += this.tablecell(token.header[j]);
        }
        header += this.tablerow({ text: cell2 });
        let body = "";
        for (let j = 0; j < token.rows.length; j++) {
          const row = token.rows[j];
          cell2 = "";
          for (let k = 0; k < row.length; k++) {
            cell2 += this.tablecell(row[k]);
          }
          body += this.tablerow({ text: cell2 });
        }
        if (body)
...
function htmlPathFor(paths, issue) {
  return import_node_path3.default.join(paths.html, `${issue.frontmatter.id}.html`);
}
function htmlUrlFor(paths, issue) {
  const target = issue ? htmlPathFor(paths, issue) : paths.htmlIndex;
  return `file://${target}`;
}
function renderIndex(paths, all) {
  const open = all.filter((i) => i.frontmatter.status === "open").sort(
    (a, b) => SEVERITY_RANK[a.frontmatter.severity] - SEVERITY_RANK[b.frontmatter.severity] || a.frontmatter.id.localeCompare(b.frontmatter.id)
  );
  const fixed = all.filter((i) => i.frontmatter.status === "fixed").sort(
    (a, b) => (b.frontmatter.fixed_at ?? "").localeCompare(a.frontmatter.fixed_at ?? "")
  );
  const archive = all.filter(
    (i) => ["wontfix", "superseded", "duplicate"].includes(i.frontmatter.status)
  ).sort(
    (a, b) => (b.frontmatter.closed_at ?? "").localeCompare(a.frontmatter.closed_at ?? "")
  );
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>Claude Issues</title><style>${BASE_CSS}</style></head>
<body>
<h1>Claude Issues \u2014 Project Ledger</h1>
<p class="muted">${import_node_path3.default.dirname(paths.root)}</p>
...
function tableFor(issues, cols) {
  if (issues.length === 0) return `<p class="empty">None.</p>`;
  const head = cols.map((c) => `<th>${headerLabel(c)}</th>`).join("");
  const rows = issues.map((i) => `<tr>${cols.map((c) => `<td>${cell(i, c)}</td>`).join("")}</tr>`).join("");
...
function linkBadges(fm) {
  ...
  if (fm.related && fm.related.length > 0) {
    parts.push(
      "related: " + fm.related.map((r) => `<a href="${r}.html">${r}</a>`).join(", ")
    );
  }
  return parts.join("; ");
}
function renderIssue(issue, all) {
  const fm = issue.frontmatter;
  const bodyHtml = marked.parse(issue.body, { async: false });
...
<div class="meta">
  ...
  const files = (fm.files ?? []).length ? `<p><b>Files:</b> ${fm.files.map((f) => `<code>${escapeHtml(f)}</code>`).join(", ")}</p>` : "";
  const links = linkBadges(fm) ? `<p><b>Links:</b> ${linkBadges(fm)}</p>` : "";
  const incoming = all.filter(
    (other) => other.frontmatter.id !== fm.id && (other.frontmatter.supersedes === fm.id || other.frontmatter.duplicate_of === fm.id || (other.frontmatter.related ?? []).includes(fm.id))
  ).map((o) => `<a href="${o.frontmatter.id}.html">${o.frontmatter.id}</a> (${o.frontmatter.status})`).join(", ");
...
// src/output.ts
function printViewerLink(paths, issue) {
  const url = htmlUrlFor(paths, issue);
  console.log(source_default.dim("View:"), source_default.cyan(url));
}
...
var require_select2 = __commonJS({
  "node_modules/prompts/lib/elements/select.js"(exports2, module2) {
    ...
    var color = require_kleur();
    var Prompt = require_prompt2();
    var { style, clear, figures, wrap: wrap3, entriesToDisplay } = require_util2();
          ...
          for (let i = startIndex; i < endIndex; i++) {
            ...
            if (v.disabled) {
              ...
              if (v.description && this.cursor === i) {
                desc = ` - ${v.description}`;
                if (prefix.length + title.length + desc.length >= this.out.columns || v.description.split(/\r?\n/).length > 1) {
                  desc = "\n" + wrap3(v.description, { margin: 3, width: this.out.columns });
                }
              }
            }
            this.outputText += `${prefix} ${title}${color.gray(desc)}
...
var require_multiselect2 = __commonJS({
  "node_modules/prompts/lib/elements/multiselect.js"(exports2, module2) {
    ...
    var { cursor } = require_src();
    var Prompt = require_prompt2();
    var { clear, figures, style, wrap: wrap3, entriesToDisplay } = require_util2();
    var MultiselectPrompt = class extends Prompt {
      constructor(opts = {}) {
        super(opts);
        this.msg = opts.message;
      ...
      renderOption(cursor2, v, i, arrowIndicator) {
        ...
        if (v.disabled) {
          ...
          if (cursor2 === i && v.description) {
            desc = ` - ${v.description}`;
            if (prefix.length + title.length + desc.length >= this.out.columns || v.description.split(/\r?\n/).length > 1) {
              desc = "\n" + wrap3(v.description, { margin: prefix.length, width: this.out.columns });
            }
          }
        }
        return prefix + title + color.gray(desc || "");
...
function link2(id, opts) {
  ...
  if (opts.supersedes) {
    ...
    if (!oldFile) {
      console.error(source_default.red("\u2717"), `Cannot supersede unknown issue ${oldId}`);
      process.exit(1);
    }
    ...
    old.frontmatter.closed_at = (/* @__PURE__ */ new Date()).toISOString();
    writeIssue(old);
    moveIssue(old, dirForStatus(paths, "superseded"));
    console.log(
      source_default.green("\u2713"),
      `${source_default.cyan(issue.frontmatter.id)} now supersedes ${source_default.cyan(oldId)} (status \u2192 superseded)`
    );
    acted = true;
  }
  if (opts.duplicateOf) {
    ...
    if (!otherFile) {
      console.error(source_default.red("\u2717"), `Cannot mark duplicate of unknown issue ${otherId}`);
      process.exit(1);
    }
    issue.frontmatter.duplicate_of = otherId;
    issue.frontmatter.status = "duplicate";
    issue.frontmatter.closed_at = (/* @__PURE__ */ new Date()).toISOString();
    writeIssue(issue);
    moveIssue(issue, dirForStatus(paths, "duplicate"));
    console.log(
      source_default.green("\u2713"),
      `${source_default.cyan(issue.frontmatter.id)} marked as duplicate of ${source_default.cyan(otherId)}`
    );
    acted = true;
  }
  if (opts.related) {
    ...
    if (!findIssueFile(paths, relId)) {
      console.error(source_default.red("\u2717"), `Cannot relate to unknown issue ${relId}`);
      process.exit(1);
    }
    ...
    console.log(source_default.green("\u2713"), `Linked ${source_default.cyan(issue.frontmatter.id)} \u2194 ${source_default.cyan(relId)}`);
    acted = true;
  }
  if (opts.unrelated) {
    ...
    console.log(source_default.green("\u2713"), `Unlinked ${source_default.cyan(issue.frontmatter.id)} \u21AE ${source_default.cyan(relId)}`);
    acted = true;
  }
  if (!acted) {
    console.error(
...
var require_utils2 = __commonJS({
  "node_modules/cli-table3/src/utils.js"(exports2, module2) {
    ...
    function mergeOptions(options3, defaults) {
      options3 = options3 || {};
      defaults = defaults || defaultOptions();
      let ret = Object.assign({}, defaults, options3);
      ret.chars = Object.assign({}, defaults.chars, options3.chars);
      ret.style = Object.assign({}, defaults.style, options3.style);
      return ret;
    }
...
var require_trap = __commonJS({
  "node_modules/@colors/colors/lib/custom/trap.js"(exports2, module2) {
    "use strict";
    module2["exports"] = function runTheTrap(text, options3) {
      var result = "";
...
var require_zalgo = __commonJS({
  "node_modules/@colors/colors/lib/custom/zalgo.js"(exports2, module2) {
    "use strict";
    module2["exports"] = function zalgo(text, options3) {
      ...
      function heComes(text2, options4) {
        var result = "";
        var counts;
        var l;
        options4 = options4 || {};
        options4["up"] = typeof options4["up"] !== "undefined" ? options4["up"] : true;
        options4["mid"] = typeof options4["mid"] !== "undefined" ? options4["mid"] : true;
        options4["down"] = typeof options4["down"] !== "undefined" ? options4["down"] : true;
        options4["size"] = typeof options4["size"] !== "undefined" ? options4["size"] : "maxi";
        text2 = text2.split("");
        for (l in text2) {
          if (isChar(l)) {
            continue;
...
var require_cell = __commonJS({
  "node_modules/cli-table3/src/cell.js"(exports2, module2) {
    ...
    var Cell = class _Cell {
      ...
      setOptions(options3) {
        if (["boolean", "number", "bigint", "string"].indexOf(typeof options3) !== -1) {
          options3 = { content: "" + options3 };
        }
        options3 = options3 || {};
        this.options = options3;
        let content = options3.content;
        ...
        this.colSpan = options3.colSpan || 1;
        this.rowSpan = options3.rowSpan || 1;
...
var require_table = __commonJS({
  "node_modules/cli-table3/src/table.js"(exports2, module2) {
    ...
    var Table2 = class extends Array {
      constructor(opts) {
        super();
        const options3 = utils.mergeOptions(opts);
        Object.defineProperty(this, "options", {
          value: options3,
          enumerable: options3.debug
        });
        if (options3.debug) {
          switch (typeof options3.debug) {
            case "boolean":
              debug.setDebugLevel(debug.WARN);
              break;
            case "number":
              debug.setDebugLevel(options3.debug);
              break;
            case "string":
              debug.setDebugLevel(parseInt(options3.debug, 10));
              break;
            default:
              debug.setDebugLevel(debug.WARN);
              debug.warn(`Debug option is expected to be boolean, number, or string. Received a ${typeof options3.debug}`);
          }
...
async function add(opts) {
  ...
  if (!opts.noScan) {
    ...
    if (matches.length > 0) {
      console.log(source_default.yellow("\u26A0"), "Similar past issues:");
      for (const m of matches) {
        console.log(
          `  ${source_default.cyan(m.frontmatter.id)} ${source_default.dim("(" + m.frontmatter.status + ")")} \u2014 ${m.frontmatter.title}`
        );
      }
      console.log(
        source_default.dim(
  ...
  const filePath = import_node_path6.default.join(paths.open, fileNameFor(id, title));
  const body = `
## Description
...
`;
  ...
  regenerateIndex(paths);
  regenerateHtml(paths);
  console.log(source_default.green("\u2713"), `Added ${source_default.cyan(id)} \u2014 ${title}`);
  console.log(`  ${source_default.dim("\u2192")} ${import_node_path6.default.relative(paths.cwd, filePath)}`);
  printViewerLink(paths, {
    frontmatter: { id },
    body: "",
    filePath
...
async function applySupersedes(newId, oldIdRaw) {
  const { link: link3 } = await Promise.resolve().then(() => (init_link(), link_exports));
  link3(newId, { supersedes: oldIdRaw });
}
function findRelated(all, title, files) {
...
init_output();
var SEV_COLORS = {
  critical: source_default.bgRed.white,
  high: source_default.red,
  medium: source_default.yellow,
  low: source_default.gray
};
var STATUS_COLORS = {
  open: source_default.cyan,
  fixed: source_default.green,
  wontfix: source_default.gray,
  superseded: source_default.magenta,
  duplicate: source_default.blueBright
};
function list2(opts) {
  const paths = resolvePaths();
  ensureInitialized(paths);
  ...
  if (issues.length === 0) {
    console.log(source_default.dim(`No ${label} issues.`));
    printViewerLink(paths);
    return;
  }
  issues.sort(
  ...
  console.log(source_default.dim(`${issues.length} ${label} issue${issues.length === 1 ? "" : "s"}`));
  printViewerLink(paths);
}
function linkSummary2(i) {
  const fm = i.frontmatter;
  const parts = [];
...
init_html();
init_output();
var FIX_NOTES_HEADER = "## Fix notes";
function appendFixNote(issue, text) {
...
function note(id, text) {
  ...
  if (!text || !text.trim()) {
    console.error(source_default.red("\u2717"), "Note text is required.");
    process.exit(1);
  }
  const issue = readIssue(file);
  ...
  regenerateIndex(paths);
  regenerateHtml(paths);
  console.log(source_default.green("\u2713"), `Note added to ${source_default.cyan(updated.frontmatter.id)}`);
...
function fix(id, opts) {
  ...
  if (issue.frontmatter.status === "fixed") {
    console.log(source_default.yellow("!"), `${issue.frontmatter.id} is already fixed.`);
    return;
  }
  ...
  regenerateIndex(paths);
  regenerateHtml(paths);
  console.log(source_default.green("\u2713"), `Fixed ${source_default.cyan(moved.frontmatter.id)} \u2014 ${moved.frontmatter.title}`);
...
function reopen(id) {
  ...
  if (issue.frontmatter.status === "open") {
    console.log(source_default.yellow("!"), `${issue.frontmatter.id} is already open.`);
    return;
  }
  ...
  regenerateIndex(paths);
  regenerateHtml(paths);
  console.log(source_default.green("\u2713"), `Reopened ${source_default.cyan(moved.frontmatter.id)} \u2014 ${moved.frontmatter.title}`);
  printViewerLink(paths, moved);
}
...
function wontfix(id, opts) {
  ...
  if (issue.frontmatter.status === "wontfix") {
    console.log(source_default.yellow("!"), `${issue.frontmatter.id} is already wontfix.`);
    return;
  }
  ...
  regenerateIndex(paths);
  regenerateHtml(paths);
  console.log(source_default.green("\u2713"), `Marked ${source_default.cyan(moved.frontmatter.id)} as ${source_default.gray("wontfix")}`);
  printViewerLink(paths, moved);
}
...
function view(id, opts) {
  ...
  console.log(source_default.cyan(url));
  if (!opts.noOpen) {
    const opener = process.platform === "darwin" ? "open" : process.platform === "win32" ? "start" : "xdg-open";
    try {
      (0, import_node_child_process.spawn)(opener, [url], { detached: true, stdio: "ignore" }).unref();
    } catch {
    }
  }
}
...
program2.command("show <id>").description("Print an issue's full markdown").action(wrap2((id) => show(id)));
program2.command("fix <id>").description("Mark an issue as fixed and move it to fixed/").option("-n, --note <text>", "Append a final fix note before closing").action(wrap2((id, opts) => fix(id, opts)));
program2.command("reopen <id>").description("Reopen any closed issue (fixed/wontfix/superseded/duplicate \u2192 open)").action(wrap2((id) => reopen(id)));
program2.command("note <id> <text>").description("Append a timestamped progress note to an issue").action(wrap2((id, text) => note(id, text)));
